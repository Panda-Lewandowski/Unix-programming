...
new_int08:
		...
		; любознательный читатель может обнаружить, что далее идёт код макроса my_print_eax(ниже), который занимается печатью ЕАХ на экран
...
new_int09:
		...
		jne	not_leave					; если не Enter - всё прекрасно, крутимся дальше
		mov escape,1					; если Enter - ставим флаг, что пора на выход
	not_leave:

		; не Enter - разрешаем обрабатывать клавиатуру
		in	al,61h
